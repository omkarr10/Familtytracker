...
CREATE TABLE IF NOT EXISTS theft_photos (
...
CREATE TABLE IF NOT EXISTS remote_commands (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  device_id UUID REFERENCES devices(id) ON DELETE CASCADE,
  command TEXT NOT NULL CHECK (command IN ('lock', 'alarm', 'capture', 'locate', 'wipe', 'activate_theft_mode', 'stop_alarm')),
...
CREATE INDEX IF NOT EXISTS idx_theft_photos_device_id ON theft_photos(device_id);
CREATE INDEX IF NOT EXISTS idx_theft_photos_captured_at ON theft_photos(captured_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_remote_commands_pending ON remote_commands(device_id, status) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_theft_events_device_id ON theft_events(device_id);
CREATE INDEX IF NOT EXISTS idx_theft_events_created_at ON theft_events(created_at DESC);

-- Enable Row Level Security
ALTER TABLE theft_photos ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Users can view their devices theft photos" ON theft_photos
  FOR SELECT USING (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );

CREATE POLICY "Devices can insert theft photos" ON theft_photos
  FOR INSERT WITH CHECK (true);

-- RLS Policies for remote_commands
CREATE POLICY "Users can view and create commands for their devices" ON remote_commands
  FOR ALL USING (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );
...
CREATE POLICY "Users can view their devices theft events" ON theft_events
  FOR SELECT USING (
    device_id IN (SELECT id FROM devices WHERE user_id = auth.uid())
  );

CREATE POLICY "Devices can insert theft events" ON theft_events
  FOR INSERT WITH CHECK (true);
...
RETURNS TRIGGER AS $$
BEGIN
  DELETE FROM theft_photos
  WHERE id IN (
    SELECT id FROM theft_photos
...
DROP TRIGGER IF EXISTS cleanup_theft_photos_trigger ON theft_photos;
CREATE TRIGGER cleanup_theft_photos_trigger
  AFTER INSERT ON theft_photos
  FOR EACH ROW
